 ...
 * CRM regression gate.
 *
 * Unlike the ERP gate, this one runs a scripted agent that actually solves each
 * task, so the recorded artifacts cover the tool bodies, the state each task
 * leaves behind and the verifier's pass path — not just its judgement on
 * pristine data.
 *
...
const drifted = await runTemplateBaseline({
  template: "crm",
  label: "CRM",
  baselineDir: new URL("./baselines/crm/", import.meta.url).pathname,
  agentPath: new URL("./crm-solver.agent.ts", import.meta.url).pathname,

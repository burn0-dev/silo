/**
 * Tool contract for the project-tracking environment.
 *
 * The contract comes from Silo. This module pins it to this environment's state
 * type and re-exports it next to the schema builders and input readers every
...
/** A project tool: a Silo tool bound to this environment's state. */
export type ProjectTool = SiloTool<State>;

/** `defineTool` with the project state type already applied. */
export function defineTool(tool: ProjectTool): ProjectTool {
  return tool;
}
...
export function readOptionalString(input: ToolInput, field: string): string | undefined {
  const value = input[field];
...
export function readBoolean(input: ToolInput, field: string, fallback: boolean): boolean {
  const value = input[field];
...
export function readEnum<T extends string>(
  ...
  if (!values.includes(value as T)) {
    throw toolError("invalid_input", `"${field}" must be one of: ${values.join(", ")}.`, {
      received: value,
    });
  }
...
/** ISO calendar date, `YYYY-MM-DD`. Rejects anything the date helpers cannot read. */
export function readDate(input: ToolInput, field: string): string {
  const value = readString(input, field);

  if (!/^\d{4}-\d{2}-\d{2}$/.test(value) || Number.isNaN(Date.parse(`${value}T00:00:00.000Z`))) {
    throw toolError("invalid_input", `"${field}" must be a date in YYYY-MM-DD format.`, {
      received: value,
    });
  }
...
export function readOptionalDate(input: ToolInput, field: string): string | undefined {
  if (input[field] === undefined || input[field] === null) {
    return undefined;
  }

  return readDate(input, field);
}

export function paginate<T>(
  items: T[],
  input: ToolInput,
): { total: number; count: number; offset: number; results: T[] } {
